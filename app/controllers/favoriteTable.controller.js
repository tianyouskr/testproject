...
exports.getFavoriteTableList=async(req,res)=>{
    ...
    try{
        ...
        const consultantIds=favoriteTables.map(favoriteTable=>favoriteTable.consultantId);
        const consultants=await Consultant.findAll({
            where:{
                id:consultantIds,
            },
            attributes:['name','workingCondition','serviceStatus','rating','introduction','price']
        });
        return res.status(200).send({
            consultants
        });
