...
exports.acceptOrder = async (req, res) => {
  // 需要修改的地方是当已经存在consultantid时，无法通过此接口接单
  try {
    ...
    (consultant.workingCondition = 'Busy'), consultant.totalOrders++;
...
exports.acceptOrderWithConsultant = async (req, res) => {
  try {
    ...
    (consultant.workingCondition = 'Busy'), consultant.completedOrders++;
    order.isResponse = true;
    order.consultantReply = consultantReply;
    order.status = 'completed';
    if (order.isUrgent) {
