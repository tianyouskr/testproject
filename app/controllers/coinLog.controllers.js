const { or, ValidationErrorItemOrigin, useInflection } = require("sequelize");
...
async function createCoinLog(orderId,reviewId,userId,consultantId,userCoin,consultantCoin,timestamp,currentUserCoinChange,currentConsultantChange,reason){
    try{
...
/*
const updateCoinLog=async(orderId)=>{
    try{
        const order=await Order.findByPk(orderId);
        if(!order){
            throw new Error("Server error");
        }
        const coinLog=await CoinLog.findOne({
          where:{
            orderId:orderId,
        ...
        if(consultantId){
            ...
            coinLog.consultantId=consultantId;
            coinLog.consultantCoin=consultant.coin;
            coinLog.currentConsultantChange=payAmount;
            await coinLog.save();
        }
    }catch(error){
        console.error(error);
...
async function consultantCoinLog(orderId){
    try{
        ...
        if(consultantId){
            ...
            await createCoinLog(orderId,0,userId,consultantId,user.coin,consultant.coin,timestamp,0,currentConsultantChange,reason);
        }
    }catch(error){
        console.error(error);
        throw new Error("Server error");
...
async function handleOrderTimeout(orderId){
    try{
        ...
        if(userId){
            ...
            await createCoinLog(orderId,0,userId,0,user.coin,0,timestamp,currentUserCoinChange,0,reason);
        }
    }catch(error){
        console.error(error);
        throw new Error("Server error");
...
async function rewardCoinLog(reviewId){
    try{
        ...
        await createCoinLog(review.orderId,reviewId,userId,consultantId,user.coin,consultant.coin,timestamp,currentUserCoinChange,currentConsultantChange,reason);      
    }catch(error){
        console.error(error);
        throw new Error("Server error");
    }
