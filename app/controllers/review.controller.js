      ...
      if (reply) {
        ...
        try {
          const reviews = await Review.findAll({
            ...
            attributes: ['rating', 'comment', 'rewardAmount', 'userName', 'orderId'],
          });
          ...
          // 将查询结果存储到Redis缓存中
          redisClient.setex(`consultantReviews:${consultantId}`, 3600, JSON.stringify(consultantReviews));
    ...
    try {
        redisClient.get(`userReviews:${userId}`, async (err, reply) => {
            if (reply) {
                ...
                const reviews = await Review.findAll({
                    where: { userId: userId },
                    attributes: ['rating', 'comment', 'rewardAmount', 'userName', 'orderId', 'consultantId']
                });
                ...
                redisClient.setex(`userReviews:${userId}`, 3600, JSON.stringify(userReviews));
